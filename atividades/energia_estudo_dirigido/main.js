// main.js — Scroll reveal, sumário ativo, reset e relatório — Energia
...
(function () {
  ...
  function initReset() {
    ...
    btn.addEventListener('click', function () {
      if (typeof window.EnergiaQuizReset === 'function') window.EnergiaQuizReset();
    });
  }
  ...
  // ================================================================
  const TOPIC_NAMES = {
    quiz1: 'Tópico 01 — O que é Energia?',
    quiz2: 'Tópico 02 — Trabalho: Transferência de Energia',
    quiz3: 'Tópico 03 — Energia Cinética',
    quiz4: 'Tópico 04 — Energia Potencial Gravitacional',
    quiz5: 'Tópico 05 — Energia Potencial Elétrica',
    quiz6: 'Tópico 06 — Conservação de Energia',
    quiz7: 'Tópico 07 — Exemplos de Conservação de Energia',
    quiz8: 'Tópico 08 — Unidades de Energia e Potência',
  };

  function stripHtml(str) { return str.replace(/<[^>]+>/g, ''); }

  function generateReport() {
    const state = window.EnergiaQuizState;
    const data  = window.EnergiaQuizData;
    ...
    const lines = [
      '╔══════════════════════════════════════════╗',
      '║  RELATÓRIO DE ESTUDO                     ║',
      '║  Energia — Física do Ensino Médio        ║',
      '╚══════════════════════════════════════════╝',
      '',
      'Data: ' + dateStr,
    ];
  ...
  function initReport() {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', function () {
        ...
        a.href = url;
        a.download = 'relatorio_energia_' + new Date().toISOString().slice(0, 10) + '.txt';
    ...
    if (emailBtn) {
      emailBtn.addEventListener('click', function () {
        const subject = encodeURIComponent('Relatório de Estudo — Energia');
