// main.js — Scroll reveal, sumário ativo, reset e relatório — Potencial Elétrico
...
(function () {
    ...
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          ...
          if (active) active.style.color = '#4361ee';
        }
      });
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', function () {
      if (typeof window.PotencialQuizReset === 'function') window.PotencialQuizReset();
    });
  }
  ...
  // ================================================================
  const TOPIC_NAMES = {
    quiz1: 'Tópico 01 — Força Elétrica e Trabalho',
    quiz2: 'Tópico 02 — Energia Potencial Elétrica',
    quiz3: 'Tópico 03 — Potencial Elétrico',
    quiz4: 'Tópico 04 — Potencial de uma Carga Pontual',
    quiz5: 'Tópico 05 — Campo Elétrico e Potencial',
    quiz6: 'Tópico 06 — Diferença de Potencial (Tensão)',
    quiz7: 'Tópico 07 — Trabalho e Diferença de Potencial',
    quiz8: 'Tópico 08 — Analogia com a Gravitação',
  };

  function stripHtml(str) { return str.replace(/<[^>]+>/g, ''); }

  function generateReport() {
    const state = window.PotencialQuizState;
    const data  = window.PotencialQuizData;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', function () {
      ...
      a.href = url;
      a.download = 'relatorio_potencial_eletrico_' + new Date().toISOString().slice(0, 10) + '.txt';
  ...
  if (emailBtn) {
    emailBtn.addEventListener('click', function () {
      const subject = encodeURIComponent('Relatório de Estudo — Potencial Elétrico');
